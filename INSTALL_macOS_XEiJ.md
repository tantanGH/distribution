...
* v1.1 2023.1.1 tantan ... フルディスクアクセス許可設定追加, XEiJ 23.01.01版対応の修正
* v1.0 2023.1.1 tantan ... 初版
...
* [HFSを使ったmacOS側とのファイルのやり取り](#hfs-%E3%82%92%E4%BD%BF%E3%81%A3%E3%81%9F-macos%E5%81%B4%E3%81%A8%E3%81%AE%E3%83%95%E3%82%A1%E3%82%A4%E3%83%AB%E3%81%AE%E3%82%84%E3%82%8A%E3%81%A8%E3%82%8A)
...
この文章執筆時点では version ~~0.22.10.18~~ 0.23.01.01 が最新です。
...
最近の macOS, 特に 10.14 Mojave以降ではセキュリティがますます厳しくなっており、自分のホーム以下にあるフォルダのアクセスもデフォルトでは制限されています。

ここでは "JavaLauncher" に対してフルディスクアクセスを許可します。

macOSの「設定」アイコンから「プライバシーとセキュリティ」「フルディスクアクセス」を選択します。
「＋」ボタンを押して以下を追加します。管理者認証を求められた場合は対応します。

    Macintosh HD → システム → ライブラリ → CoreServices → JavaLauncher.app

![](https://github.com/tantanGH/distribution/raw/main/images/xeij21.png)

これで XEiJ にフルディスクアクセス権を与えることができます。
...
~~問題となるのは macOS 側で英語キーボードを使っている場合です。~~ 

XEiJの0.23.01.01版で macOS 英語キーボードの対応がなされました。
当方から提供させて頂いたマッピングを取り込んで頂けました。ありがとうございます。

XEiJの「入力」メニューから「キー割り当て」を選択し、US配列にチェックを入れて「初期値に戻す」を行えばokです。
...
この方法については [こちら](https://yatte-mita.hateblo.jp/entry/2019/06/15/130053) の情報を参考にさせて頂きました。ありがとうございます。



以下の作業は XEiJ 0.22.10.21版以前で必要だったものです。0.23.01.01版以降を利用しましょう。

    これは物理的には日本語キーボードなのに英語キーボード配列を選択しているケースも含みます。ちなみに自分はこのパターンなのですが、なぜAppleからの購入時に英語キーボードモデルではなく日本語キーボードモデルを選択したかというと、「英数」「かな」
