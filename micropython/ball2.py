...
class SpriteUtil:
  ...
  def init(self, clear_and_on=True):
    ...
    if self.__class__.s is None:
      self.__class__.s = x68k.Sprite()
...
#    x68k.iocs(x68k.i.SPALET, d1=pack('L',(v<<31)|code), d2=block, d3=color_code)
    SpriteUtil.s.palet(code, color_code, block, False)
...
#    x68k.iocs(x68k.i.SP_DEFCG, d1=code, d2=size, a1=uctypes.addressof(bytes(pattern)))
    SpriteUtil.s.defcg(code, bytes(pattern), size)
...
#    machine.mem16[ REG_SP_SCROLL + sprite_no * 8 + 6 ] = prw
    ...
    SpriteUtil.s.set(sprite_no, 16 + x, 16 + y, (pl_block << 8) | pt_code, prw, False)
...
# vsync callback
def vsync_callback(balls):

  for b in balls:
    if b.invalid is True:
      b.scroll()
      b.invalid = False
...
def main():
  ...
  x68k.iocs(x68k.i.BOX,a1=pack('6h',0,0,511,511,0b10000_10000_00000_0,0xffff))
