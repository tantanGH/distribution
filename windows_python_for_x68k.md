...
(手元のWindowsが英語版なので英語表記になっていますが、日本語版Windowsではもしかしたら日本語になっているかもしれません。適宜読み替えてください。)
...
減色アルゴリズムにも限界があるので、あまり色が多く使われていない元画像を選択するのがポイントです。


---

### Python サンプルの利用 (dim2xdf)

X680x0 エミュレータで使われる FDイメージの形式には主に `.dim` と `.xdf` の2つの形式があります。
`.dim` はヘッダ情報がありサイズ可変なのに対して、`.xdf` はシンプルにベタデータです。
...
### Python サンプルの利用 (wav2adpcm)

.wav ファイルを X680x0 ADPCM の .pcm ファイルに変換するコンバータは沢山存在します。
X680x0 (Human68k) 上で動作するものも、Windowsで動作するものもあります。

しかし macOS で動くものはなさそうなので Python で書きました。

このように Python で書くと Windows だろうが macOS だろうが Linux だろうが、どこでも動くマルチプラットフォームなアプリケーションを簡単に
作れてしまうのは大きなメリットと言えます。

    wav2adpcm [option] <input.wav> <output.pcm>

使い方は入力となる wav ファイルと 出力となる pcm ファイルを指定するだけです。
オプションは2つあり、`-f` でローパスフィルターの有無を指定可能です。

    -f1 ... ローパスフィルター有効(デフォルト)
    -f0 ... ローパスフィルター無効
    
X680x0 のADPCMは15.625kHzなので、理論上は7~8kHzの音までしか再生できません。
wav データは 44.1kHz とか 48kHz の物が多く、X680x0 ADPCMで再生できない高周波成分を含めて無理やり変換してしまうと音質低下の元になります。
これを防ぐためにローパスフィルターを緩くかけています。

このローパスフィルターの機能も `pydub` という既存ライブラリの機能を活用しています。

もう一つ　`-v` オプションがあり、すぐ後ろにつけた数字でレベル(ボリューム)の上げ下げがdB単位でできます。
古めの wav データや古めの CDからリッピングしたデータなどはややレベルが控えめになっていることが多く、
これをX680x0の 4bit ADPCMに変換すると情報量がさらに減ってしまいます。こういう場合はやや上げ目にしてから変換した方が良い結果になります。

実際の用途としては、
 - 音楽wavデータを変換してX680x0環境で聴く
 - フリーの音楽素材サイトからドラムのサンプリングWAVデータをダウンロードしてそれを変換してX680x0の自作曲に使う
 - MACSなどの動画の素材データ変換処理を処理能力のあるホスト(Windows)側で実行し、かつ自動化する
...
以上ざっくりとした流れで説明しましたが、ぜひみなさんもお手軽かつパワフルなPythonを活用してみてください。
