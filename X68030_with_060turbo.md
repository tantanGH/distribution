...
* [060loadhigh利用上の注意](#060loadhigh-%E5%88%A9%E7%94%A8%E4%B8%8A%E3%81%AE%E6%B3%A8%E6%84%8F)
...
060turbo.sys のマニュアルには68060モードの時にFLOATn.Xを組み込んではいけないと記載されています。しかし我が家の68LC060環境ではFLOAT2.Xを組み込まないと割り算した瞬間に固まりますw ということでウチではFLOAT2.X必須なのでした。
...
060turbo 関連ソフトウェアの一つに M.Kamada-san 作の 060loadhigh.x があります。

[060loadhigh.x](http://retropc.net/x68000/software/hardware/060turbo/060loadhigh/)

これはハイメモリ領域にプログラムをロードして実行するためのソフトウェアです。

    060loadhigh hogehoge.x

のようにして実行します。この時に注意点として、**060turbo.sys で拡張モードを有効にしていること** が前提条件となります。
拡張モードはHuman68kのメモリ管理をハイメモリ領域まで拡張するためのモードで `-xm` オプションを 060turbo.sys 登録時に与えることで利用可能になります。

拡張モードが有効になっていないと 060loadhigh.x は黙ってメインメモリにロードして実行します。(`-s`オプションをつけている場合はエラーにしてくれます)

拡張モードが有効になっていなくても、RAMDISK や HIMEM_ALLOC などのアロケートは行うことができるので最初気がつきませんでしたw

---

## 更新履歴

- 2023.02.18 ... 060loadhigh関連追加
- 2023.02.04 ... FPU関連追加
- 2023.02.01 ... 初版
